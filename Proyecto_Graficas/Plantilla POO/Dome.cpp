...
void Dome::LoadVertex()
{
	...
	for(int x = 0; x < slides; x++)
	{
		for(int y = 0; y < slices; y++)
		{
				...
				vertexArray[arrayIndex].x = (float)(radius * cos(((float)x / (float)(slides - 1)) * PI / 2.0f) * cos(2.0f * PI * (float)y / (float)(slices - 1)));
				vertexArray[arrayIndex].y = (float)(radius * sin(((float)x / (float)(slides - 1)) * PI / 2.0f));
				vertexArray[arrayIndex].z = (float)(radius * cos(((float)x / (float)(slides - 1)) * PI / 2.0f) * sin(2.0f * PI * (float)y / (float)(slices - 1)));
